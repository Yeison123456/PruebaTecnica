...
class usuario{
    ...
    static async listar(nro_documento){
        const query=`Select numeroDocumentoUsuario, nombresUsuario, apellidosUsuario, nombreCiudad as idCiudadFKUsuario,fechaNacimientoUsuario, emailUsuario, edadUsuario, telefonoUsuario, estadoUsuario,viabilidad, ocupacion as idOcupacionFKUsuario from usuario inner join ocupaciones on usuario.idOcupacionFKUsuario=ocupaciones.idOcupacion inner join ciudades on usuario.idCiudadFKUsuario=ciudades.idCiudad where numeroDocumentoUsuario like '%${nro_documento}'; `;
